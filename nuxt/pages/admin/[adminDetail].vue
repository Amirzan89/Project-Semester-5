<template>
    ...
    <section v-else class="tw-w-full tw-relative" style="min-height: calc(100vh - 7rem - (2rem + 20px) - 20px);">
        <div class="col-12">
            <div class="">
                <h1 class="3xs:tw-text-3xl lg:tw-text-4xl tw-font-semibold">Detail Admin</h1>
            ...
            <form class="tw-relative tw-bg-white tw-border-palBg tw-border-1 tw-rounded-xl tw-mt-5 3xs:tw-p-3 xs:tw-p-4 sm:tw-p-7 md:tw-p-8 xl:tw-p-7 sm:tw-pb-3 md:tw-pb-4 xl:tw-pb-5" style="border: 1px solid #e2e8f0; box-shadow: rgba(145,158,171,0.2) 0px 0px 2px 0px, rgba(145,158,171,0.12) 0px 12px 24px -4px;">
                <div class="3xs:tw-w-full lg:tw-w-[75%] xl:tw-w-[78%] 2xl:tw-w-[82%] tw-flex tw-grow tw-flex-col md:tw-gap-0.5 xl:tw-gap-2 3xs:tw-mt-30 lg:tw-mt-0">
                    ...
                    <InputText type="text" ref="inpNamaLengkap" :disabled="local.isMode == 'detail'" class="3xs:tw-rounded-md xl:tw-rounded-lg 3xs:tw-outline-0 3xs:focus:tw-outline-1 xl:focus:tw-outline-3 tw-text-palText tw-border-palText hover:tw-border-palText focus:tw-border-palText focus:tw-outline-palText" :class="{ 'tw-bg-palBg': local.isMode == 'detail'}" v-model="input.nama_lengkap" @input="inpChange('nama_lengkap')"/>
                </div>
                <div class="3xs:tw-w-full lg:tw-w-[75%] xl:tw-w-[78%] 2xl:tw-w-[82%] tw-mt-6 3xs:tw-mt-2 xl:tw-mt-4 2xl:tw-mt-5 tw-flex 3xs:tw-flex-col md:tw-flex-row 3xs:tw-gap-2 lg:tw-gap-5">
                    <div class="tw-flex tw-grow tw-flex-col md:tw-gap-0.5 xl:tw-gap-2.5">
                        ...
                        <Dropdown v-model="input.jenis_kelamin" ref="inpJenisKelamin" :disabled="local.isMode == 'detail'" :options="jenisKelaminValue" optionLabel="name" optionValue="cosValue" class="tw-w-full tw-h-9 tw-flex tw-justify-center tw-items-center 3xs:tw-rounded-md xl:tw-rounded-lg 3xs:tw-outline-0 3xs:focus:tw-outline-1 xl:focus:tw-outline-3 tw-text-palText tw-border-palText tw-outline-palText hover:tw-border-palText focus:tw-border-palText focus:tw-outline-palText" :class="{ 'tw-bg-palBg': local.isMode == 'detail'}" placeholder="Pilih Jenis Kelamin" @input="inpChange('jenis_kelamin')">
                    ...
                    <div class="tw-flex tw-grow tw-flex-col md:tw-gap-0.5 xl:tw-gap-2">
                        ...
                        <InputText type="text" ref="inpNoTelpon" :disabled="local.isMode == 'detail'" class="tw-w-full 3xs:tw-rounded-md xl:tw-rounded-lg 3xs:tw-outline-0 3xs:focus:tw-outline-1 xl:focus:tw-outline-3 tw-text-palText tw-border-palText hover:tw-border-palText focus:tw-border-palText focus:tw-outline-palText" :class="{ 'tw-bg-palBg': local.isMode == 'detail'}" v-model="input.no_telpon" @input="formatNoAdmin($event); inpChange('no_telpon')" />
                    </div>
                </div>
                <div class="tw-w-full tw-mt-6 3xs:tw-mt-2 sm:tw-mt-2.5 xl:tw-mt-4 2xl:tw-mt-5 tw-flex 3xs:tw-flex-col lg:tw-flex-row 3xs:tw-gap-2 lg:tw-gap-5">
                    <div class="tw-flex tw-grow tw-flex-col md:tw-gap-0.5">
                        ...
                        <InputText type="email" ref="inpEmail" :disabled="local.isMode == 'detail'" class="tw-w-full 3xs:tw-rounded-md xl:tw-rounded-lg 3xs:tw-outline-0 3xs:focus:tw-outline-1 xl:focus:tw-outline-3 tw-text-palText tw-border-palText hover:tw-border-palText focus:tw-border-palText focus:tw-outline-palText" :class="{ 'tw-bg-palBg': local.isMode == 'detail'}" v-model="input.email" @input="inpChange('email')"/>
                    </div>
                    <div class="tw-flex tw-grow tw-flex-col md:tw-gap-0.5">
                        <label for="password" class="">Password</label>
                        <div class="tw-relative tw-w-full tw-flex tw-items-center">
                            <InputText type="password" ref="inpPassword" :disabled="local.isMode == 'detail'" class="tw-w-full 3xs:tw-rounded-md xl:tw-rounded-lg 3xs:tw-outline-0 3xs:focus:tw-outline-1 xl:focus:tw-outline-3 tw-text-palText tw-border-palText hover:tw-border-palText focus:tw-border-palText focus:tw-outline-palText" :class="{ 'tw-bg-palBg': local.isMode == 'detail'}" @input="inpChange('password')" v-model="input.password"/>
                ...
                <div class="tw-absolute tw-w-[15%] 3xs:tw-w-[70%] 2xs:tw-w-[62%] xs:tw-w-[60%] phone:tw-w-[45%] sm:tw-w-[40%] md:tw-w-[31%] lg:tw-w-[22%] xl:tw-w-[20%] 2xl:tw-w-[17%] 3xs:tw-h-[18%] sm:tw-h-[22%] md:tw-h-[24%] lg:tw-h-[35%] 3xs:tw-top-[4%] lg:tw-top-[15%] xl:tw-top-[14%] 2xl:tw-top-[12%] 3xs:tw-left-1/2 3xs:-tw-translate-x-1/2 lg:tw-translate-x-0 lg:tw-right-[3%] xl:tw-right-[2.2%] 2xl:tw-right-[1.2%] lg:tw-ml-auto tw-object-cover tw-flex tw-flex-col tw-justify-center tw-gap-2 tw-rounded-lg" :class="{
                    'tw-cursor-pointer' : local.isMode == 'edit',
                    'tw-border-black tw-border-dashed tw-border-3' : input.linkImg === ''
                }" @dragover.prevent="handleDragOverPersonal" @drop.prevent="handleDropPersonal" @click="handleFormClickPersonal">
                    <img :src="input.linkImg" alt="" class="tw-w-full tw-h-full tw-object-contain tw-absolute" :class="{ 'tw-hidden': input.linkImg === ''}">
                    <I_Drop class="tw-mt-2 tw-h-15 tw-relative tw-top-2 tw-pointer-events-none" :class="{ 'hidden': input.linkImg !== ''}"/>
                    <span class="tw-text-center tw-text-lg" :class="{ 'tw-hidden': input.linkImg !== ''}">Pilih atau jatuhkan file gambar</span>
                ...
                <div class="tw-relative 3xs:tw-w-full md:tw-w-max tw-h-max md:tw-right-0 md:tw-ml-auto 3xs:tw-mt-6 sm:tw-mt-8 xl:tw-mt-10 2xl:tw-mt-12 3xs:tw-mb-1.5 sm:tw-mb-2 md:tw-mb-3 tw-flex 3xs:tw-justify-between md:tw-gap-5 tw-text-palText">
                    <template v-if="local.isMode == 'detail'">
                        <Button class="tw-w-max tw-flex tw-justify-center tw-items-center tw-text-palBg 3xs:tw-px-1.5 xs:tw-px-2 md:tw-px-3 lg:tw-px-3 xl:tw-px-3 3xs:tw-text-sm xs:tw-text-base sm:tw-text-lg lg:tw-text-xl" @click="local.isMode = 'detail'">
                            <NuxtLink to="/admin" class="tw-text-palBg tw-w-full tw-h-full tw-flex tw-justify-center tw-items-center">Kembali</NuxtLink>
                        </Button>
                        <Button class="tw-w-max 3xs:tw-py-0 3xs:tw-px-3 lg:tw-px-3 tw-text-palBg 3xs:tw-text-base sm:tw-text-lg xl:tw-text-xl" @click.prevent="local.isMode == 'detail' ? local.isMode = 'edit' : editAdminForm();">Edit</Button>
                    </template>
                    <template v-else>
                        <Button class="tw-w-max 3xs:tw-py-0 3xs:tw-px-3 lg:tw-px-3 tw-text-palBg 3xs:tw-text-base sm:tw-text-lg xl:tw-text-xl" @click="local.isMode = 'detail'">Batal</Button>
                        <Button class="tw-flex tw-justify-around tw-items-center 3xs:tw-gap-1 sm:tw-gap-2 xl:tw-gap-2.5 tw-w-max 3xs:tw-px-2 lg:tw-px-3 tw-text-palBg" @click.prevent="local.isMode == 'detail' ? local.isMode = 'edit' : editAdminForm();">
                            <I_Edit class="3xs:tw-w-5 xs:tw-w-6 sm:tw-w-7 lg:tw-w-8"/>
                            <span class="3xs:tw-text-base sm:tw-text-lg">Simpan</span>
                        </Button>
                    </template>
                </div>
            </form>
...
import { EditAdmin } from '~/composables/api/admin';
...
import I_Edit from '~/assets/icons/edit.svg';
...
const local = reactive({
    ...
    isMode: 'detail' as 'detail' | 'edit',
    isUpdated: false,
    isPasswordShow: false,
    fetchedViewData: null as any,
});
useHead({
    title:`${local.isMode.toUpperCase()} Admin | ${publicConfig.appName}`
});
...
const bNavItems = ref([
    ...
    { label: 'Detail Admin', url: `/admin/${route.params.adminDetail}` },
]);
...
watch(() => local.fetchedViewData, () => {
    if (local?.fetchedViewData !== undefined && local.fetchedViewData !== null && typeof local.fetchedViewData === 'object' && !Array.isArray(local.fetchedViewData) && Object.keys(local.fetchedViewData).length > 0) {
        input.nama_lengkap = local.fetchedViewData.nama_lengkap;
        input.jenis_kelamin = local.fetchedViewData.jenis_kelamin;
        input.no_telpon = local.fetchedViewData.no_telpon;
        input.role = local.fetchedViewData.role;
        input.email = local.fetchedViewData.email;
        input.email_old = local.fetchedViewData.email;
        input.linkImg = `${publicConfig.baseURL}/admin/download/foto/${route.params.adminDetail as string}`;
    }
}, { immediate: true });
...
const handleFiles = async(files: FileList) => {
    ...
    input.foto = file;
    input.linkImg = URL.createObjectURL(file);
};
...
const editAdminForm = async() => {
    if(local.isRequestInProgress) return;
    let errMessage = '';
    if (input.nama_lengkap === local.fetchedViewData.nama_lengkap && input.email === local.fetchedViewData.email && input.jenis_kelamin === local.fetchedViewData.jenis_kelamin && input.no_telpon === local.fetchedViewData.no_telpon && input.foto === null) {
        if(!errMessage) errMessage = 'Data belum diubah !';
    }
    if(errMessage != ''){
        return toast.add({ severity: 'error', summary: 'Gagal Update Admin', detail: errMessage, group: 'br', life: 3000 })
    }
    const isValidForm = checkValidator();
    if(isValidForm.status == 'error'){
        inpChange(isValidForm.item as cosValidationType, true);
        return toast.add({ severity: 'error', summary: 'Gagal Update Admin', detail: isValidForm.message, group: 'br', life: 3000 })
    }
    let formData = {
        id_admin: Array.isArray(route.params.adminDetail) ? route.params.adminDetail[0] : route.params.adminDetail,
        nama_lengkap: input.nama_lengkap,
        jenis_kelamin: input.jenis_kelamin,
        no_telpon: input.no_telpon,
        email_old: input.email_old,
        email: input.email !== local.fetchedViewData.email ? input.email : undefined,
        foto: input.foto !== null ? input.foto : undefined,
    }
    local.isRequestInProgress = true;
    eventBus.emit('showLoading');
    let res = await EditAdmin(formData);
    if(res.status === 'success'){
        local.fetchedViewData = input;
        local.isRequestInProgress = false;
        eventBus.emit('closeLoading');
        toast.add({ severity: 'success', summary: 'Berhasil Update Admin', detail: res.message, group: 'br', life: 3000 });
        local.isMode = 'detail';
    }else if(res.status === 'error'){
        local.isRequestInProgress = false;
        eventBus.emit('closeLoading');
        toast.add({ severity: 'error', summary: 'Gagal Update Admin', detail: res.message, group: 'br', life: 3000 });
